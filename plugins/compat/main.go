...
func (p *CompatPlugin) SetModelCatalog(mc *modelcatalog.ModelCatalog) {
	p.modelCatalog = mc
}
...
func (p *CompatPlugin) PostLLMHook(ctx *schemas.BifrostContext, result *schemas.BifrostResponse, bifrostErr *schemas.BifrostError) (*schemas.BifrostResponse, *schemas.BifrostError, error) {
	...
	if len(droppedParams) > 0 {
		if result != nil && result.ChatResponse != nil {
			result.ChatResponse.ExtraFields.DroppedCompatPluginParams = droppedParams
		}

		if result != nil && result.TextCompletionResponse != nil {
			result.TextCompletionResponse.ExtraFields.DroppedCompatPluginParams = droppedParams
		}
	}
