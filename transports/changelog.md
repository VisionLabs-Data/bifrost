    ...
    "virtual_keys": [
      {
        "id": "vk-unrestricted",
        "provider_configs": [
          { "provider": "openai",    "allowed_models": ["*"], "weight": 1.0 },
          { "provider": "anthropic", "allowed_models": ["*"], "weight": 1.0 },
          { "provider": "azure",     "allowed_models": ["*"], "weight": 1.0 }
        ]
      }
    ]
  }
...
The field used to restrict which provider API keys a Virtual Key can use has been renamed from `allowed_keys` to `key_ids`. The semantics have also been updated to match the new whitelist model:

| Value | v1.4.x behavior | v1.5.0 behavior |
|---|---|---|
| Field absent / `[]` | Allow all keys | **Allow all keys** (uses `allow_all_keys: true` internally) |
| `["key-1"]` | Only key-1 | Only key-1 (unchanged) |
| `["*"]` | Not applicable | Allow all keys (explicit wildcard) |

<Note>
The rename of `allowed_keys` → `key_ids` is the primary change here. The "allow all when empty" behavior is **preserved** for `key_ids` (unlike `allowed_models`), because an empty `key_ids` is internally resolved to `allow_all_keys: true`. However, you should migrate to the explicit `["*"]` form for clarity.
</Note>
...
<Step title="Rename allowed_keys to key_ids">
Search your `config.json` and API calls for `"allowed_keys"` and rename it to `"key_ids"`. No value change needed — just the field name.
</Step>
